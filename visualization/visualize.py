...
previous_polarities = []
def update(frame, scat, quiv, line, positions, angles, time_steps, vel, L):

    if len(previous_polarities) > frame:
        return scat, quiv, line

    # Each 10 % of the simulation, print the progress
...
def create_animation(time_steps, positions, angles, L, vel, noise, rc, output_file='data/animation.mp4'):
    ...
    scat = ax.scatter([], [], s=10)  # Small size for points
    ...
    # Set up mean angle plot (bottom plot)
    ...
    ani = animation.FuncAnimation(
        ...
        fargs=(scat, quiv, line, positions, angles, time_steps, vel, L),
        interval=interval,
        blit=True
    )
